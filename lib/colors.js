/**
 * SVG colors.  Inspired by CairoSVG https://github.com/Kozea/CairoSVG/edit/master/cairosvg/colors.py
 */
const COLORS = {
...
const RGB_REGX = new RegExp(/^rgb\([ \n\r\t]*(.+?)[ \n\r\t]*\)$/)
const RGBA_REGX = new RegExp(/^rgba\([ \n\r\t]*(.+?)[ \n\r\t]*\)$/)
const HEX_RGB_REGX = new RegExp(/^#([0-9a-f]{3})$/)
const HEX_RRGGBB_REGX = new RegExp(/^#([0-9a-f]{6})$/)

module.exports = function getColor (str, opacity = 1) {
  ...
  if (str in COLORS) {
    const [r, g, b, a] = COLORS[str]
    return [r, g, b, a * opacity]
  }

  if (str.match(RGB_REGX)) {
    const [r, g, b] = str.match(RGB_REGX)[1].split(',')
    return [parseFloat(r / 255), parseFloat(g / 255), parseFloat(b / 255), opacity]
  }

  if (str.match(RGBA_REGX)) {
    const [r, g, b, a] = str.match(RGBA_REGX)[1].split(',')
    return [parseFloat(r / 255), parseFloat(g / 255), parseFloat(b / 255), parseFloat(a * opacity)]
  }

  if (str.match(HEX_RGB_REGX)) {
    let [r, g, b] = str.match(HEX_RGB_REGX)[1].split('')
    r = parseInt(r + r, 16)
    g = parseInt(g + g, 16)
    b = parseInt(b + b, 16)
    return [parseFloat(r / 255), parseFloat(g / 255), parseFloat(b / 255), opacity]
  ...
  if (str.match(HEX_RRGGBB_REGX)) {
    let [r, r1, g, g1, b, b1] = str.match(HEX_RRGGBB_REGX)[1].split('')
    r = parseInt(r + r1, 16)
    g = parseInt(g + g1, 16)
    b = parseInt(b + b1, 16)
    return [parseFloat(r / 255), parseFloat(g / 255), parseFloat(b / 255), opacity]
